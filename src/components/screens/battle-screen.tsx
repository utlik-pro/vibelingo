...
export function BattleScreen({ onStartBattle }: BattleScreenProps) {
  ...
  return (
    <div className="px-5 py-4 bg-white min-h-full">
      <h2 className="text-[22px] font-extrabold text-gray-900 mb-1.5">{t("battle.title")}</h2>
      <p className="text-xs text-gray-400 mb-5">{t("battle.subtitle")}</p>

      {/* Battle card */}
      <div className="rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.06)] mb-4 animate-[slideIn_0.4s_ease]">
        <div className="px-6 py-8 bg-gradient-to-br from-purple-50 to-purple-100 text-center">
          ...
          <h3 className="text-lg font-bold text-gray-900 mb-1">{t("battle.quickBattle")}</h3>
        ...
        <div className="px-5 py-5">
          <div className="flex justify-between mb-4">
            <span className="text-xs text-gray-400">{t("battle.reward")}</span>
      ...
      <div
        ...
        className="flex items-center gap-3 p-4 rounded-2xl bg-white border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)] mb-4 cursor-pointer hover:border-purple-200 transition-colors animate-[slideIn_0.4s_ease_0.1s_both]"
      >
        <div className="w-11 h-11 rounded-xl bg-purple-50 flex items-center justify-center">
          <Target className="w-5 h-5 text-purple-500" />
        </div>
        <div className="flex-1">
          <div className="text-[15px] font-semibold text-gray-900">{t("battle.challengeFriend")}</div>
      ...
      >
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs text-gray-400">{t("battle.participants", { n: 47 })}</span>
